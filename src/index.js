...
const testers = (() => {
	function test() {
		if (todolist.list.length === 0) {
			let storeTask = todoitem(
				"Nip down to the store",
				"Get a funny feeling",
				"2020-07-11",
				"Low",
				"incomplete"
			);
			let lotteryTask = todoitem(
				"Buy lottery ticket",
				"No. Could it be?",
				"2021-07-11",
				"Mid",
			...
			let runTask = todoitem(
				"Run home",
				"Darling! Get off the phone! Everything has changed!",
				"2022-07-11",
				"High",
				"incomplete"
			);
			let finishedTask = todoitem(
				"Avoid housework",
				"'You know it's no good for my asthma.'",
				"2020-01-01",
				"High",
				"complete"
			);
